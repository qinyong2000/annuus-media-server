...
public class Mp4Deserializer implements IMediaDeserializer {
	...
	public Mp4Deserializer(RandomAccessFileReader reader) {
		...
		if (trak != null) {
			videoTrak = trak;
			videoSamples = trak.getAllSamples(MediaSample.SAMPLE_VIDEO);
			samples.addAll(Arrays.asList(videoSamples));
		}
		trak = moov.getAudioTrak();
		if (trak != null) {
			audioTrak = trak;
			audioSamples = trak.getAllSamples(MediaSample.SAMPLE_AUDIO);
	...
	public MediaSample metaData() {
		...
		return new MediaSample(MediaSample.SAMPLE_META, 0, AmfValue.toBinary(AmfValue.array("onMetaData", value)));
	}
	...
	public MediaSample readNext() throws IOException {
		if (sampleIndex < samples.size()) {
			...
			if (sample.isVideoSample()) {
				return new MediaSample(MediaSample.SAMPLE_VIDEO, sample.getTimestamp(), createVideoTag(sample));
			}
			if (sample.isAudioSample()) {
				return new MediaSample(MediaSample.SAMPLE_AUDIO, sample.getTimestamp(), createAudioTag(sample));
			}
		}
